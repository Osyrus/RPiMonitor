...
class Pane:
    """docstring for Pane"""
    def __init__(self, idStr, info, instrType, LCal, x = 1, y = 1, w = 30, h = 1):
        self.x, self.y = 0, 0
        self.w, self.h = 0, 0
        self.setPos(x, y)
        self.setDim(w, h)
        ...
        self.idNum = self.idStr[len(self.idStr) - 1]
    ...
    def mqttPublish(self, mqttC, topic, data):
        mqttC.publish(topic + self.idStr, str(time.time()) + " " + str(data), 0, retain=False)

class IVIPane(Pane):
    """docstring for IVIPane"""
    def __init__(self, idStr, info, readCommand, LCal = (1, 0)):
        Pane.__init__(self, idStr, info, "ivi", LCal)
    ...
    def update(self, iviInstr, mqttC):
        ...
        self.mqttPublish(mqttC, "laser/sensors/", data)
